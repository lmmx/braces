...
use super::trie::Node;
use super::normalise::{can_stem_split, clean_trailing_sep, find_common_string_prefix, find_common_string_suffix};
...
pub fn compute_reprs(
    ...
    sep: &str,
    config: &BraceConfig,
) -> (std::collections::HashMap<usize, String>, std::collections::HashMap<usize, Vec<String>>) {
    ...
    for &idx in &post {
        ...
        for (child_label, &child_idx) in node.children.iter() {
            child_repr_items.push(reprs.get(&child_idx).cloned().unwrap_or_else(|| child_label.clone()));
        ...
        let node_raws: Vec<String> = if node.children.is_empty() {
            if node.label.is_empty() { vec![String::new()] } else { vec![node.label.clone()] }
        } else {
            let mut out = vec![];
            for r in child_raws.iter() {
                if node.label.is_empty() { out.push(r.clone()) }
                else if r.is_empty() { out.push(node.label.clone()) }
                else { out.push(format!("{}{}{}", node.label, sep, r)); }
            }
            if node.is_leaf { out.push(node.label.clone()) }
            out
        };
        ...
        let repr = if node.depth > config.max_depth {
            ...
            let suffixes: Vec<String> = node_raws.iter().map(|s| {
                if node.label.is_empty() { s.clone() }
            ...
            if config.disallow_empty_braces && suffixes.iter().any(|s| s.is_empty()) && suffixes.len() > 1 {
                // Don't use braces, list separately
                node_raws.join(" ")
            } else {
                compose_label_and_items(&node.label, sep, &suffixes, config.max_brace_size, !config.preserve_order_within_braces)
            }
        } else {
            let mut items = child_repr_items.clone();
            if node.is_leaf { items.push(String::new()) }
            ...
            if config.disallow_empty_braces && items.iter().any(|s| s.is_empty()) && items.len() > 1 {
                ...
                let mut vars = items.iter().map(|s| s.strip_prefix(&prefix).unwrap_or(s).strip_suffix(&suffix).unwrap_or(s).to_string()).collect::<Vec<_>>();
                if !config.preserve_order_within_braces { vars.sort(); }
                let inner = if vars.len() == 1 { vars[0].clone() } else { format!("{{{}}}", vars.join(",")) };
                if node.label.is_empty() { format!("{}{}{}", prefix, inner, suffix) }
                ...
                compose_label_and_items(&node.label, sep, &items, config.max_brace_size, !config.preserve_order_within_braces)
            }
        };
...
fn compose_label_and_items(label: &str, sep: &str, items: &[String], max_brace_size: Option<usize>, sort_items: bool) -> String {
    let mut cleaned: Vec<String> = items.iter().map(|s| clean_trailing_sep(s, sep).to_string()).collect();
    if sort_items { cleaned.sort(); }

    let compose_inner = |slice: &[String]| if slice.len() == 1 { slice[0].clone() } else { format!("{{{}}}", slice.join(",")) };

    if let Some(max) = max_brace_size {
        if cleaned.len() <= max {
            if label.is_empty() { compose_inner(&cleaned) } else { format!("{}{}{}", label, sep, compose_inner(&cleaned)) }
        } else {
            let mut groups = vec![];
            for chunk in cleaned.chunks(max) {
                if label.is_empty() { groups.push(compose_inner(chunk)) } else { groups.push(format!("{}{}{}", label, sep, compose_inner(chunk))) }
            }
            groups.join(" ")
        }
    } else {
        ...
        if label.is_empty() {
            ...
            if has_empty && cleaned.len() > 1 {
                ...
                let formatted: Vec<String> = cleaned.iter().map(|s| {
                    if s.is_empty() { String::new() } else { format!("{}{}", sep, s) }
                }).collect();
...
pub fn expand_braces(pattern: &str) -> Vec<String> {
    if !pattern.contains('{') { return vec![pattern.to_string()]; }
    ...
    while let Some(ch) = chars.next() {
        if ch == '{' {
            ...
            while let Some(c) = chars.next() {
                ...
                else if c == '}' { depth -= 1; if depth == 0 { break; } inner.push(c); }
                else { inner.push(c); }
            }
    ...
    for r in &mut results { r.push_str(&current); }
    results
}
